...
# verify
echo ""
echo "This will drop the database! Are you sure!!?? (yes/no)"
read VERIFY
if [ $VERIFY != 'yes' ]; then return; fi

echo ""
echo "cloud-run.job.db-drop script: run container as the db create job..."
...
JOB=db-drop-job$GCP_PROJECT_RESOURCE_NAMING_SUFFIX
